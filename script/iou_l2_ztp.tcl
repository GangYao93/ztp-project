#!/usr/bin/tclsh
# IOU L2 Tcl bootstrap and controller registration.
#
# DHCP option 67 must identify iou_l2_autoinstall.cfg. AutoInstall applies
# that IOS configuration, whose one-shot EEM applet runs this file directly
# from TFTP with tclsh.
...
set MANAGEMENT_PORT "Ethernet0/0"
set MANAGEMENT_VLAN 1
set MANAGEMENT_INTERFACE "Vlan1"
set INSTALL_APPLET "INSTALL-IOU-L2-ZTP"
...
set mac_address [get_interface_mac $MANAGEMENT_PORT]
set hostname "IOU-L2"
if {$mac_address != ""} {
    set compact_mac [string map {":" ""} $mac_address]
    set hostname "IOU-L2-[string toupper [string range $compact_mac end-5 end]]"
}
...
set json_body [format {{"ip_address":"%s","mac":"%s","os_type":"cisco_ios","status":"register","device_type":"switch","username":"%s","password":"%s"}} \
